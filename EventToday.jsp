	...
	<div class="main_content">
		<div class="container">
			...
			<nav>
				...
				<ul class="pagination">
					<li><a
						href="ListEventNowServlet?page=
						<c:out value="${start}"/>"
						aria-label="前のページへ"> <span aria-hidden="true">«</span>
					</a></li>

					<c:forEach var="i" begin="${start}" end="${end}">
						<li <c:if test="${event.page==i}">class="active"</c:if>
						><a href="ListEventNowServlet?page=<c:out value="${i}"/>"><c:out
									value="${i}" /></a></li>
					</c:forEach>
					<li><a href="ListEventNowServlet?page=<c:out value="${end}"/>"
